...
1. The files `android/app/google-services.json`, `ios/Runner/GoogleService-Info.plist`, and `lib/firebase_options.dart` should be auto-generated by the flutterfire cli tool when connecting to the firebase project. To use your own firebase application, replace them with the new files created using the flutterfire CLI (`flutterfire configure`).
